...
hasParent = function (schObj) {
	return (["PAGE", "COLLECTIONS", "CONTAINER"].some(function (elem) {
		return elem == schObj.dataType;
	}));
};

isContainer = function (schObj) {
	return (["PAGE", "CONTAINER"].some(function (elem) {
		return elem == schObj.dataType;
	}));
};
...
valWithSch = function (recSch, rec) {
	...
	if (Array.isArray(recSch)) {

		for (var s = 0; s < recSch.length; s++) {
			var recSchObj = recSch[s];
			var recDataObj = rec[recSchObj.name] 
			if (hasParent(recSchObj)) {
				if (isContainer(recSchObj)) {
					var dataKeys = getJsonKeys(rec);
					var dataSchemaList = getJsonKeysFromSchema(recSch);

					var err=hasValidKeys( dataKeys, dataSchemaList);
					if (err != null) { 
						return err;
					}

					// checkMandatory list
					// checkContainer has option to add freeField is true -- new feature

					var err = validateContainer(recSchObj, recDataObj);
					if (err != null) { 
						return err;
					}
				...
				var s = 0;
				var dataKeys = getJsonKeys(rec);
				if (hasValidKeys(recSchKeys, dataKeys)) {
					return new Error("Unexpected value= " + JSON.stringify(dataKeys) + ", expected values are recSchKeys=" + JSON.stringify(recSchKeys));
				}
				var rtErr = doDataValidate(rec, recSch, s);
				if (rtErr != null) {
					return rtErr;
				}
			}
...
validateContainer = function (recSchObj, rec) {
	//var recSchKeys = getJsonKeysFromSchema(recSch);

	 if (isObject(rec) && isObject(recSchObj)) {
		recSchObj.childs =  recSchObj.childs || [];

		for (var s=0; s< recSchObj.childs.length; s++) {
			var childSchObj= recSchObj.childs[s];
			var value = rec[childSchObj.name];

			if (isContainer(childSchObj)) {
				var rtErr = validateContainer(childSchObj, value);
				if (rtErr != null) {
					return rtErr;
				}
			} else {

				var rtErr = doDataValidateObj(childSchObj, value);
				if (rtErr != null) {
					return rtErr;
				}
			}
		...
		var rtErr = new Error("container and data json has some issue");
		if (rtErr != null) {
			return rtErr;
		}
	 }
	...
	switch (recSch[s].dataType) {
		...
		case "OPTION":
			...
			if (value != '') {
				...
				inpStrArr.forEach(function (obj, index) {
					if (index % 2 == 0) {
						values.push(obj)
					}
				});
...
function doDataValidateObj(recSchObj, value) {
	value = new String(value);
	switch (recSchObj.dataType) {
		case "DATE":
			re = /^\d{1,2}\/\d{1,2}\/\d{4}$/;
			if ((value != '') && (!value.match(re))) {
				return new Error(recSchObj.name + " has invalid " + recSchObj.dataType + " format: [" + value + "]");
			}
			break;
		case "TIME":
			re = /^\d{1,2}:\d{2}([ap]m)?$/;
			...
			break;
		case "NUMBER":
			if (recSchObj.max != "unlimited") {
				re = RegExp("^[0-9.]{" + recSchObj.min + "," + recSchObj.max + "}$");
				if (value != '' && !re.test(value)) {
					return new Error(recSchObj.name + " has invalid " + recSchObj.dataType + " format: [" + value + "]");
				}
			} else {
		...
		case "VARCHAR":
			if (recSchObj.max != "unlimited") {
				re = RegExp("^[A-Za-z0-9_\.\\s]{" + recSchObj.min + "," + recSchObj.max + "}$");
				//console.log(typeof value);
				//console.log(value);
				if (value != '' && !value.match(re)) {
					return new Error(recSchObj.name + " has invalid " + recSchObj.dataType + " format: [" + value + "]");
		...
		case "OPTION":
			...
			if (value != '') {
				var inpStrArr = recSchObj.listVal.split('|');
				var chk = 0;
				var values = new Array();
				inpStrArr.forEach(function (obj, index) {
					if (index % 2 == 0) {
				...
				if (!values.some(function (elem) {
					...
					return new Error(recSchObj.name + " has invalid " + recSchObj.dataType + " value : [" + value + "] expected: " + JSON.stringify(values));
				}
			}
	}
	return null;
