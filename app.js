	...
	addCoreFunction(req, function (req) {
		...
		var DataJson = {
			"ServiceDetails": {
				...
				"serviceName": req.params.module
			}
		}
		log.info("calling InvokeDB for serviceDetails");
		err = heaeriesjson.valWithSch(SchemaJson, DataJson);
		if (err) {
		...
		idb.InvokeDB(pageId, pageType, SchemaJson, DataJson, SchemaJson, function (err, respSchemaJson, respDataJson) {
			...
			findService(respDataJson.ServiceDetails.services, req.params.task, req.getMethod(), function (err, currentService) {
				if (err) {
