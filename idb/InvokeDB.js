...
var  InvokeDB = function(pageId, pageType, SchemaJson, DataJson, calback) {
	if (heaeriesjson.valWithSch(DataJson, SchemaJson)) {
			require('./map/' + mapper[pageId].map)[pageType](SchemaJson, DataJson,  calback);
	} else {
			throw next(new Error("Schema Validation Failed"));
...
var DataJson=[{"ServiceDetails":[{"services":[{"resSjson":"Response schema json","reqSjson":"Request schema json","authReqd": false ,"task":"getUserDetails"}],"serviceName":""}]}];
