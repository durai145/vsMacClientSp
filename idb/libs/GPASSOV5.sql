...
-- -----------------------------------------------------
-- Table `default_schema`.`TASK007MT`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `default_schema`.`TASK007MT` ;
...
CREATE TABLE IF NOT EXISTS `GPASSO`.`SRVS006MT` (
  ...
  `TASK_IDS` INT(11) NULL,
  PRIMARY KEY (`_ID`),
  CONSTRAINT `RK_TASK007MT_TASK_IDS`
    FOREIGN KEY (`TASK_IDS`)
    REFERENCES `default_schema`.`TASK007MT` (`_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
