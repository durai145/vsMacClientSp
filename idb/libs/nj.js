...
if (process.argv.length != 4) {
	console.error("node %s <input.sql> <type> " + process.argv[1]);
	process.exit(1);
}
...
fs.readFile(inputSqlFile, 'utf8', function (err, data) {
	...
	sqlObj.forEach(function (tableObj) {
		if (inputType == "mongoose") {
			prepareMongoosObj(tableObj)
		} else {
			preparJavaObj(tableObj)
		}
	});
});
...
function preparJavaObj(tableObj) {
	...
	createFolder(packagePath);
	basePathFile = packagePath + "/" + modelName + ".java"



	var package = util.format("package %s;\n", base);

	var

		javaFile = util.format("/*  Generated from %s by nj.js  Don't change */\n", inputSqlFile);
	javaFile += package;

	packageImport = ""
	packageImportList = new Set();
...
function createFile(path, data) {
	fs.open(path, 'w+', (err, fd) => {
		...
		writtenData = fs.writeSync(fd, data);
		console.log("Written data %s %s", path, writtenData);
	})
}

function prepareGetterAndSetter(column) {

	outStr = "";
	outStr += util.format("\n\tpublic %s get%s() {\n ", column.Type, ClassCamleCase(column.Name));
	outStr += util.format("\t\treturn %s ; \n", camleCase(column.Name))
	outStr += util.format("\t} \n")


	outStr += util.format("\n\tpublic void set%s(%s %s) {\n", ClassCamleCase(column.Name), column.Type, camleCase(column.Name));
	outStr += util.format("\t\tthis.%s = %s;\n", camleCase(column.Name), camleCase(column.Name))
	outStr += util.format("\t} \n")
	return outStr;
}
...
checkAndAdd = function (packName, packageImportList) {
	packageImportList.add(packName);
}

function prepareMongoosObj(tableObj) {
