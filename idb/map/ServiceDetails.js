...
exports.getServiceDetails = function (inSchema, inJson, inRespSchema, callback) {
	...
	GPASSO_PAGE005MT_Model.find({
		"pageKey": inJson.ServiceDetails.serviceName
	}, function (err, srvs006mt) {
		var outJson = {
			"ServiceDetails": {
				"services": [{
					"resSjson": [],
		...
		if ((srvs006mt.length === undefined) || (srvs006mt.length == 0)) {
			return callback(new Error("No data found [ page_key=" + inJson.ServiceDetails.serviceName + "]"));

		}
// services should be multiple
		outJson.ServiceDetails.services[0].resSjson = srvs006mt[0].services[0].resSjson;
		outJson.ServiceDetails.services[0].reqSjson = srvs006mt[0].services[0].reqSjson;
		outJson.ServiceDetails.services[0].authReqd = srvs006mt[0].services[0].authReqd;
		outJson.ServiceDetails.services[0].task = srvs006mt[0].services[0].task;
		outJson.ServiceDetails.services[0].method = srvs006mt[0].services[0].method;
		outJson.ServiceDetails.serviceName = srvs006mt[0].pageKey;
...
exports.saveServiceDetails = function (inSchema, inJson, inRespSchema, callback) {
	...
	service.save(function (err) {
		if (err) {
			...
			return callback && callback(null, ErrorResponseSchema, [{
				"ErrorResponse": [{
					"status": [{
						"responseCode": "001",
						"responseDesc": "unable to save in database"
					}]
				}]
			}]);
		}
		return callback && callback(null, ErrorResponseSchema, [{
			"ErrorResponse": [{
				"status": [{
					"responseCode": "000",
					"responseDesc": "Successfully saved"
				}]
			}]
		}]);
